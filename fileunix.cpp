...
#if defined(OS_QNX) || defined(OS_BEOS) || defined(OS_HAIKU) ||                \
    defined(OS_MPEIX)
#define NO_AR
#define HAVE_AR
#endif
...
void file_query_(file_info_t *const info) { file_query_posix_(info); }
...
void file_archscan(char const *arch, scanback func, void *closure) {
  ...
  /* Report the collected archive content. */
  {
    ...
    for (; iter != end; iter = filelist_next(iter)) {
       ...
       */
      sprintf(buf, "%s(%s)", object_str(archive->file->name),
...
void file_archivescan_(file_archive_info_t *const archive,
                       archive_scanback func, void *closure) {}
...
int file_collect_archive_content_(file_archive_info_t *const archive) {
  ...
  while ((read(fd, &ar_hdr, SARHDR) == SARHDR) &&
    ...
    int32_t ar_hdr_name_size =
        sizeof(ar_hdr.ar_name); // Workaround for sizeof strncpy warning.
...
static void collect_archive_content_small(int fd,
                                          file_archive_info_t *const archive) {
...
static void collect_archive_content_big(int fd,
                                        file_archive_info_t *const archive) {
...
int file_collect_archive_content_(file_archive_info_t *const archive) {
  ...
  if (read(fd, fl_magic, SAIAMAG) != SAIAMAG || lseek(fd, 0, SEEK_SET) == -1) {
    close(fd);
    return -1;
  }
