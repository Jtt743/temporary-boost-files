...
#define GET_WAIT_FD(job_idx)                                                   \
  (wait_fds + ((job_idx * (globs.pipe_action ? 2 : 1))))
...
void exec_init(void) {
  ...
  if (globs.jobs > cmdtab_size) {
    ...
    memset(cmdtab + cmdtab_size, 0,
           (globs.jobs - cmdtab_size) * sizeof(*cmdtab));
    wait_fds =
        (pollfd *)BJAM_REALLOC(wait_fds, WAIT_FDS_SIZE * sizeof(*wait_fds));
...
int exec_check(string const *command, LIST **pShell, int32_t *error_length,
  ...
  return is_raw_cmd
             ? EXEC_CHECK_OK
             : check_cmd_for_too_long_lines(command->value, shell_maxline(),
...
void exec_cmd(string const *command, int flags, ExecCmdCallback func,
  ...
  if (!default_shell)
    default_shell =
        list_push_back(list_new(object_new("/bin/sh")), object_new("-c"));
  ...
  if (cmdtab[slot].pid == 0) {
    ...
    dup2(globs.pipe_action ? err[EXECCMD_PIPE_WRITE] : out[EXECCMD_PIPE_WRITE],
         STDERR_FILENO);
...
static int read_descriptor(int i, int s) {
  ...
  while (0 <
         (ret = fread(buffer, sizeof(char), BUFSIZ - 1, cmdtab[i].stream[s]))) {
    buffer[ret] = 0;
  ...
  while (!finished) {
    ...
    if (globs.timeout > 0) {
      ...
      for (i = 0; i < globs.jobs; ++i)
        if (cmdtab[i].pid) {
          long const consumed = (long)(current - cmdtab[i].start_time) / tps;
    ...
    for (i = 0; i < globs.jobs; ++i) {
      ...
      if (out_done || err_done) {
        ...
        if (WIFEXITED(status))
          cmdtab[i].exit_reason = WEXITSTATUS(status) ? EXIT_FAIL : EXIT_OK;

        {
          time_info.system = ((double)(cmd_usage.ru_stime.tv_sec) * 1000000.0 +
                              (double)(cmd_usage.ru_stime.tv_usec)) /
                             1000000.0;
          time_info.user = ((double)(cmd_usage.ru_utime.tv_sec) * 1000000.0 +
                            (double)(cmd_usage.ru_utime.tv_usec)) /
                           1000000.0;
...
int32_t shell_maxline() { return MAXLINE; }
